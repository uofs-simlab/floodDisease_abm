...
# Headless full-compound launcher for lab/HPC machines.
...
log_file="${log_dir%/}/full_compound_${ts}.log"

echo "[RUN] Repo: $repo_root"
echo "[RUN] Log: $log_file"
echo "[RUN] Command: python run/full_compound.py $*"

set +e
python run/full_compound.py "$@" 2>&1 | tee "$log_file"

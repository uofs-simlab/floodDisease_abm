...
# Headless flood-and-mold launcher for lab/HPC machines.
...
log_file="${log_dir%/}/flood_vectorborne_${ts}.log"

echo "[RUN] Repo: $repo_root"
echo "[RUN] Log: $log_file"
echo "[RUN] Command: python run/flood_vectorborne.py $*"

set +e
python run/flood_vectorborne.py "$@" 2>&1 | tee "$log_file"

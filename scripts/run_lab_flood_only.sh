...
# Headless flood-only launcher for lab/HPC machines.
...
log_file="${log_dir%/}/flood_only_${ts}.log"

echo "[RUN] Repo: $repo_root"
echo "[RUN] Log: $log_file"
echo "[RUN] Command: python run/flood_only.py $*"

set +e
python run/flood_only.py "$@" 2>&1 | tee "$log_file"

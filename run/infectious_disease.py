...
def build_common_kwargs(args) -> dict:
    return build_common_model_kwargs(
        ...
        extra_kwargs={
            "scenario_mode": "infectious_disease",
...
def run_one_replication(task: dict) -> dict:
    ...
    kwargs["run_id"] = f"infectious_disease_rep{task['replication']:03d}_seed{task['seed']}"
...
def orchestrate_remote_infectious_disease(args):
    """
    Full end-to-end orchestration:
    1. Sync and launch remote batch run
    2. Poll remote _progress.json until complete
    3. Download results via SCP
    4. Generate infectious-disease graphs locally
    """
    print("[ORCHESTRATION] Starting remote infectious-disease pipeline...")
    target = args.remote_target.lower()
    user = args.remote_user
    password = args.remote_password
    remote_repo = args.remote_repo

    experiment_name = f"infectious_disease_{args.persons}x{args.replications}"
    remote_out_dir = f"outputs/infectious_disease/{experiment_name}"
    ...
    try:
        ...
        if not launch_remote_batch(
            ...
            runner="infectious_disease",
            session_name=f"infectious_disease_{args.persons}x{args.replications}_{target}",
    ...
    if args.out_dir:
        ...
        local_out_dir = (ROOT / "outputs" / "infectious_disease" / experiment_name).resolve()
    ...
    print("[4/4] Generating infectious-disease graphs...")
    try:
        graph_script = ROOT / "analysis" / "generate_infectious_disease_graphs.py"
        if not graph_script.exists():
            ...
            cmd = [
                ...
                "--scenario", "infectious_disease",
                "--out-subdir", "infectious_disease_graphs",
            ]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
            if result.returncode == 0:
                print(f"[OK] Graphs generated in {local_out_dir}/infectious_disease_graphs/")
            else:
...
def main():
    parser = argparse.ArgumentParser(description="Run infectious-disease replicated batches with clean infectious-disease outputs.")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "ROOT/outputs/infectious_disease/infectious_disease_<persons>x<replications>."
        ),
    )
    parser.add_argument(
        "--keep-rep-folders",
        action="store_true",
        help="Keep infectious-disease rep_XXX folders. By default, replication folders are cleaned up after aggregation.",
    )
    parser.add_argument(
        "--gottlieb",
        action="store_true",
    ...
    if args.remote:
        success = orchestrate_remote_infectious_disease(args)
    ...
    if Model is None or export_summary is None or export_timeseries is None or pd is None:
        ...
        print(f"[ERR] Cannot run local infectious-disease: {reason}")
        print("[ERR] For remote orchestration, use: python run/infectious_disease.py --persons 350 --replications 12 --gottlieb")
        return 1
    ...
    if args.out_dir:
        ...
        experiment_name = f"infectious_disease_{args.persons}x{args.replications}"
        out_root = (ROOT / "outputs" / "infectious_disease" / experiment_name).resolve()
    ...
    print(f"Running infectious-disease batch with {args.replications} replications")
            ...
            while pending:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        print(
                            f"  FAILED (infectious-disease rep {task['replication'] + 1}, "
                            f"seed={task['seed']}): {type(exc).__name__}: {exc}"
                        )
                        continue
                    print(
                        f"  completed {completed}/{len(tasks)} "
                        f"(infectious-disease rep {task['replication'] + 1}, seed={task['seed']})"
                    )
                    results.append(result)
    ...
    if not results:
        raise RuntimeError("All infectious-disease replications failed; no results were produced")
    ...
    stats_df = aggregate_summary_rows(summary_rows, default_scenario="infectious_disease")
    ...
    print(f"Saved infectious-disease outputs to {out_root}")
    return 0

...
def run_one_replication(task: dict) -> dict:
    ...
    kwargs["run_id"] = f"floodonly_rep{task['replication']:03d}_seed{task['seed']}"
...
def orchestrate_remote_flood_only(args):
    """Run the flood-only pipeline remotely, monitor progress, pull results, and generate graphs."""
    print("[ORCHESTRATION] Starting remote flood-only pipeline...")
    target = args.remote_target.lower()
    user = args.remote_user
    password = args.remote_password
    remote_repo = args.remote_repo

    experiment_name = f"floodonly_{args.persons}x{args.replications}"
    remote_out_dir = f"outputs/flood_only/{experiment_name}"
    ...
    try:
        ...
        if not launch_remote_batch(
            ...
            runner="flood_only",
            session_name=f"floodonly_{args.persons}x{args.replications}_{target}",
    ...
    if args.out_dir:
        ...
        local_out_dir = (ROOT / "outputs" / "flood_only" / experiment_name).resolve()
    ...
    print("[4/4] Generating flood-only graphs...")
    try:
        graph_script = ROOT / "analysis" / "generate_floodonly_graphs.py"
        if not graph_script.exists():
            ...
            cmd = [
                ...
                "--scenario", "flood_only",
                "--out-subdir", "floodonly_graphs",
            ]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
            if result.returncode == 0:
                print(f"[OK] Graphs generated in {local_out_dir}/floodonly_graphs/")
            else:
...
def main():
    parser = argparse.ArgumentParser(description="Run flood-only replicated batches with clean flood-only outputs.")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "ROOT/outputs/flood_only/floodonly_<persons>x<replications>."
        ),
    )
    parser.add_argument(
        "--keep-rep-folders",
        action="store_true",
        help="Keep flood-only rep_XXX folders. By default, replication folders are cleaned up after aggregation.",
    )
    parser.add_argument(
        "--gottlieb",
        action="store_true",
    ...
    if args.remote:
        success = orchestrate_remote_flood_only(args)
        return 0 if success else 1

    if Model is None or export_summary is None or export_timeseries is None:
        print(f"[ERR] Cannot run local flood-only: {IMPORT_ERROR}")
        print("[ERR] For remote orchestration, use: python run/flood_only.py --persons 350 --replications 12 --gottlieb")
        return 1
    ...
    if args.out_dir:
        ...
        experiment_name = f"floodonly_{args.persons}x{args.replications}"
        out_root = (ROOT / "outputs" / "flood_only" / experiment_name).resolve()
    ...
    print(f"Running flood-only batch with {args.replications} replications")
            ...
            while pending:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        print(
                            f"  FAILED (flood-only rep {task['replication'] + 1}, "
                            f"seed={task['seed']}): {type(exc).__name__}: {exc}"
                        )
                        continue
                    print(
                        f"  completed {completed}/{len(tasks)} "
                        f"(flood-only rep {task['replication'] + 1}, seed={task['seed']})"
                    )
                    results.append(result)
    ...
    if not results:
        raise RuntimeError("All flood-only replications failed; no results were produced")
    ...
    stats_df = aggregate_summary_rows(summary_rows, default_scenario="flood_only")
    ...
    print(f"Saved flood-only outputs to {out_root}")
    return 0

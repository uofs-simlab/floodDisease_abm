...
def build_common_kwargs(args) -> dict:
    return build_common_model_kwargs(
        ...
        extra_kwargs={
            "scenario_mode": "flood_vectorborne",
        },
    )
...
def run_one_replication(task: dict) -> dict:
    ...
    kwargs["run_id"] = f"flood_vectorborne_rep{task['replication']:03d}_seed{task['seed']}"
...
def orchestrate_remote_flood_vectorborne(args):
    """Run the flood-vectorborne pipeline remotely, monitor progress, pull results, and generate graphs."""
    print("[ORCHESTRATION] Starting remote flood-and-vectorborne pipeline...")
    target = args.remote_target.lower()
    user = args.remote_user
    password = args.remote_password
    remote_repo = args.remote_repo

    experiment_name = f"flood_vectorborne_{args.persons}x{args.replications}"
    remote_out_dir = f"outputs/flood_vectorborne/{experiment_name}"
    ...
    try:
        ...
        if not launch_remote_batch(
            ...
            runner="flood_vectorborne",
            session_name=f"flood_vectorborne_{args.persons}x{args.replications}_{target}",
    ...
    if args.out_dir:
        ...
        local_out_dir = (ROOT / "outputs" / "flood_vectorborne" / experiment_name).resolve()
    ...
    print("[4/4] Generating flood-and-vectorborne graphs...")
    try:
        graph_script = ROOT / "analysis" / "generate_flood_vectorborne_graphs.py"
        if not graph_script.exists():
            ...
            cmd = [
                ...
                "--scenario", "flood_vectorborne",
                "--out-subdir", "flood_vectorborne_graphs",
            ]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
            if result.returncode == 0:
                print(f"[OK] Graphs generated in {local_out_dir}/flood_vectorborne_graphs/")
            else:
...
def main():
    parser = argparse.ArgumentParser(description="Run flood-and-vectorborne replicated batches with clean flood-and-vectorborne outputs.")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "ROOT/outputs/flood_vectorborne/flood_vectorborne_<persons>x<replications>."
        ),
    )
    parser.add_argument(
        "--keep-rep-folders",
        action="store_true",
        help="Keep flood-and-vectorborne rep_XXX folders. By default, replication folders are cleaned up after aggregation.",
    )
    parser.add_argument(
        "--gottlieb",
        action="store_true",
    ...
    if args.remote:
        success = orchestrate_remote_flood_vectorborne(args)
        return 0 if success else 1

    if Model is None or export_summary is None or export_timeseries is None:
        print(f"[ERR] Cannot run local flood-and-vectorborne: {IMPORT_ERROR}")
        print("[ERR] For remote orchestration, use: python run/flood_vectorborne.py --persons 350 --replications 12 --gottlieb")
        return 1
    ...
    if args.out_dir:
        ...
        experiment_name = f"flood_vectorborne_{args.persons}x{args.replications}"
        out_root = (ROOT / "outputs" / "flood_vectorborne" / experiment_name).resolve()
    ...
    print(f"Running flood-and-vectorborne batch with {args.replications} replications")
            ...
            while pending:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        print(
                            f"  FAILED (flood-vectorborne rep {task['replication'] + 1}, "
                            f"seed={task['seed']}): {type(exc).__name__}: {exc}"
                        )
                        continue
                    print(
                        f"  completed {completed}/{len(tasks)} "
                        f"(flood-and-vectorborne rep {task['replication'] + 1}, seed={task['seed']})"
                    )
                    results.append(result)
    ...
    if not results:
        raise RuntimeError("All flood-vectorborne replications failed; no results were produced")
    ...
    stats_df = aggregate_summary_rows(summary_rows, default_scenario="flood_vectorborne")
    ...
    print(f"Saved flood-and-vectorborne outputs to {out_root}")
    return 0

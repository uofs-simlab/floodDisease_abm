...
# Try to import the model and exporters for local runs; fall back cleanly for orchestration-only use.
try:
    from dataCollection._dataCollect import export_summary, export_timeseries
    from model._model import Model
except ImportError as e:
    # Allow orchestration mode to proceed even if local imports fail.
    Model = None
    export_summary = None
...
def run_one_replication(task: dict) -> dict:
    ...
    kwargs["run_id"] = f"baseline_rep{task['replication']:03d}_seed{task['seed']}"
...
def orchestrate_remote_baseline(args):
    """Run the baseline pipeline remotely, monitor progress, pull results, and generate graphs."""
    import subprocess
    
    print("[ORCHESTRATION] Starting remote baseline pipeline...")
    target = args.remote_target.lower()
    user = args.remote_user
    password = args.remote_password
    remote_repo = args.remote_repo
    
    # Determine the remote output directory name.
    experiment_name = f"baseline_{args.persons}x{args.replications}"
    remote_out_dir = f"outputs/baseline/{experiment_name}"
    
    # Step 1: sync and launch the remote batch run.
    ...
    try:
        ...
        # Build the batch arguments expected by the remote script.
        batch_args = (
        ...
        if not launch_remote_batch(
            ...
            runner="baseline",
            session_name=f"baseline_{args.persons}x{args.replications}_{target}",
            ...
            if result.stdout.strip():
                print(result.stdout)
        print(f"[OK] Remote run launched")
    except Exception as e:
        print(f"[ERR] Could not launch remote run: {e}")
        return False
    
    # Step 2: poll remote progress until the run completes.
    print(f"[2/4] Polling remote progress (every 5 min)...")
    poll_interval = 300
    max_polls = 240
    poll_count = 0
    ...
    while poll_count < max_polls:
        ...
        if progress is None:
            ...
            if missed_progress_checks >= max_missed_progress_checks:
                print("[WARN] Progress polling unavailable; continuing with pull and graph generation.")
                break
            continue
        missed_progress_checks = 0
        ...
        if percent >= 100.0:
            print(f"[OK] Remote run completed!")
            break
    else:
    ...
    if args.out_dir:
        ...
        local_out_dir = (ROOT / "outputs" / "baseline" / experiment_name).resolve()
    ...
    if not pull_results_from_remote(
        ...
        print(f"[ERR] Failed to pull results")
        return False
    
    # Step 4: generate graphs.
    print(f"[4/4] Generating baseline graphs...")
    try:
        graph_script = ROOT / "analysis" / "generate_baseline_graphs.py"
        if not graph_script.exists():
            print(f"[WARN] Graph script not found: {graph_script}")
        else:
            # Results are pulled directly to local_out_dir.
            cmd = [
                sys.executable,
                str(graph_script),
            ...
            if result.returncode == 0:
                print(f"[OK] Graphs generated in {local_out_dir}/baseline_graphs/")
            else:
...
def main():
    parser = argparse.ArgumentParser(description="Run baseline-only replicated batches with clean baseline outputs.")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "ROOT/outputs/baseline/baseline_<persons>x<replications>."
        ),
    )
    parser.add_argument(
        "--keep-rep-folders",
        action="store_true",
        help="Keep baseline rep_XXX folders. By default, replication folders are cleaned up after aggregation.",
    )
    # HPC convenience flags (shortcuts for --remote --remote-target <hpc>)
    parser.add_argument(
        "--gottlieb",
        action="store_true",
    ...
    parser.add_argument(
        "--remote",
        action="store_true",
        help="Enable orchestration: sync code, run remotely, poll progress, pull results, and generate graphs.",
    )
    parser.add_argument(
        "--remote-target",
        type=str,
    ...
    # Process HPC convenience flags (--gottlieb, --richardson, --hundsdorfer)
    if args.gottlieb:
        args.remote = True
    ...
    # If orchestration mode is enabled, handle full remote pipeline
    if args.remote:
        success = orchestrate_remote_baseline(args)
        return 0 if success else 1

    # For local execution, verify required imports are available
    if Model is None or export_summary is None or export_timeseries is None:
        print(f"[ERR] Cannot run local baseline: {IMPORT_ERROR}")
        print("[ERR] For remote orchestration, use: python run/baseline.py --persons 350 --replications 12 --gottlieb")
        return 1
    ...
    if args.out_dir:
        ...
        experiment_name = f"baseline_{args.persons}x{args.replications}"
        out_root = (ROOT / "outputs" / "baseline" / experiment_name).resolve()
    ...
    print(f"Running baseline batch with {args.replications} replications")
            ...
            while pending:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        print(
                            f"  FAILED (baseline rep {task['replication'] + 1}, "
                            f"seed={task['seed']}): {type(exc).__name__}: {exc}"
                        )
                        continue
                    print(
                        f"  completed {completed}/{len(tasks)} "
                        f"(baseline rep {task['replication'] + 1}, seed={task['seed']})"
                    )
                    results.append(result)
    ...
    if not results:
        raise RuntimeError("All baseline replications failed; no results were produced")
    ...
    stats_df = aggregate_summary_rows(summary_rows, default_scenario="baseline")
    ...
    print(f"Saved baseline outputs to {out_root}")
    return 0

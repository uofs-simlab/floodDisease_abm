...
def build_common_kwargs(args) -> dict:
    compound_infectious_start_hour = infectious_start_hour(
        "compound", args.baseline_days, args.pre_flood_days, args.flood_days
    )
    return build_common_model_kwargs(
        args,
        resolve_data_dir(ROOT, args.map),
        extra_kwargs={
            "scenario_mode": "compound",
            "enable_infectious": 1,
            "enable_stagnant": 1,
            "enable_mold": 1,
            "infectious_seed_start_hour": compound_infectious_start_hour,
        },
    )
...
def run_one_replication(task: dict) -> dict:
    ...
    kwargs["run_id"] = f"full_compound_rep{task['replication']:03d}_seed{task['seed']}"
...
def run_local_full_compound(args, out_root: Path) -> bool:
    if Model is None or export_summary is None or export_timeseries is None or pd is None:
        reason = IMPORT_ERROR or PANDAS_IMPORT_ERROR or "Missing local Python dependency"
        print(f"[ERR] Cannot run local full-compound: {reason}")
        print("[ERR] For remote orchestration, use: python run/full_compound.py --persons 350 --replications 12 --gottlieb")
        return False
    ...
    print(f"Running full-compound batch with {args.replications} replications")
            ...
            while pending:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        print(
                            f"  FAILED (full-compound rep {task['replication'] + 1}, "
                    ...
                    print(
                        ...
                        f"(full-compound rep {task['replication'] + 1}, seed={task['seed']})"
                    )
                    results.append(result)
    ...
    if not results:
        raise RuntimeError("All full-compound replications failed; no results were produced")
    ...
    stats_df = aggregate_summary_rows(summary_rows, default_scenario="full_compound")
    ...
    print(f"Saved full-compound outputs to {out_root}")
    return True


def orchestrate_remote_full_compound(args) -> bool:
    print("[ORCHESTRATION] Starting remote full-compound pipeline...")
    target = args.remote_target.lower()
    user = args.remote_user
    password = args.remote_password
    remote_repo = args.remote_repo

    experiment_name = f"full_compound_{args.persons}x{args.replications}"
    remote_out_dir = f"outputs/full_compound/{experiment_name}"
    ...
    try:
        ...
        if args.gov_baseline_grant_every_hours is not None:
            batch_args += f" --gov-baseline-grant-every-hours {args.gov_baseline_grant_every_hours}"
        ...
        if not launch_remote_batch(
            ...
            runner="full_compound",
            session_name=f"full_compound_{args.persons}x{args.replications}_{target}",
    ...
    if args.out_dir:
        ...
        local_out_dir = (ROOT / "outputs" / "full_compound" / experiment_name).resolve()
    ...
    print("[4/4] Generating full-compound graphs...")
    try:
        graph_script = ROOT / "analysis" / "generate_full_compound_graphs.py"
        cmd = [
            sys.executable,
            str(graph_script),
            "--run-dir",
            ...
            "full_compound",
            "--out-subdir",
            "full_compound_graphs",
        ]
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
        if result.returncode == 0:
            print(f"[OK] Graphs generated in {local_out_dir}/full_compound_graphs/")
        else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run full-compound replicated batches (flood + mold + vectorborne + infectious).")
    ...
    parser.add_argument("--gov-baseline-grant-every-hours", type=int, default=None)
    parser.add_argument("--out-dir", type=str, default=None)
    parser.add_argument("--keep-rep-folders", action="store_true")
    parser.add_argument("--gottlieb", action="store_true")
    parser.add_argument("--richardson", action="store_true")
    parser.add_argument("--hundsdorfer", action="store_true")
    parser.add_argument("--remote", action="store_true")
    parser.add_argument("--remote-target", type=str, default="gottlieb", choices=["gottlieb", "richardson", "hundsdorfer"])
    parser.add_argument("--remote-user", type=str, default="oaa721")
    parser.add_argument(
        "--remote-password",
        type=str,
    ...
    parser.add_argument("--remote-repo", type=str, default="floodDisease_abm")
    ...
    if args.remote:
        return 0 if orchestrate_remote_full_compound(args) else 1

    if args.out_dir:
        out_root = Path(args.out_dir).resolve()
    else:
        out_root = (ROOT / "outputs" / "full_compound" / f"full_compound_{args.persons}x{args.replications}").resolve()

    ok = run_local_full_compound(args, out_root)
